# Flyway Migration 운영 규칙 (Dr-Judge)

> 대상: 백엔드 3인 협업 (Java 17 · Spring Boot · MySQL 8 · Flyway)
> 목적: 담당자별 버전 분리 + out-of-order 허용 + 운영 배포 안정성
> 기준: 닥터저지 ERD를 **V1 baseline**으로 고정, 이후 담당자별 메이저 버전(`V2`~`V4`)
...
| 담당자 | 버전 영역 | 예시 |
|-----| --- | --- |
| 김현지 | `V2.x.y` | `V2.2.2__...sql` |
| 임수영 | `V3.x.y` | `V3.1.1__...sql` |
| 조은서 | `V4.x.y` | `V4.1.1__...sql` |
...
**`V1`은 baseline(`V1__init_schema.sql`) 전용이며 누구에게도 배정하지 않는다.
...
> 담당자당 메이저 버전 1개 체계에서는 **레인 번호가 낮은 담당자일수록** 이 제약을 더 자주 만난다(예: `V2` 레인은 항상 `V3`~`V4`보다 낮다).
...
**닥터저지 공용 테이블** = `users`, `categories`, `archive_items` 등 여러 도메인이 함께 참조하는 핵심 테이블.
...
- **운영 환경변수 (EC2의 운영용 `.env` 파일, `compose.prod.yml`이 주입)** — 문서상의 원칙을 설정으로도 강제한다.
  ...
  > 설정하지 않으면 공통 기본값(`out-of-order=true`)이 운영에도 그대로 적용된다. **`.env`에 반드시 명시할 것.**
